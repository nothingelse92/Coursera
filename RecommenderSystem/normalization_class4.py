...
def csv_out():
    ...
    for m in range(1,6):
        ...
        for k in range(1,len(users)):
            if(users[k] == ans_out[m][2]):
                co_user = k
        num = 0
        for k in range(1,len(lines[co_user])):
            if(lines[co_user][k] != ''):
                sum += float(lines[co_user][k])
                num += 1
        ave_s.insert(m,sum/num)
    sum = 0
    num = 0
    for k in range(1,len(lines[14])):
        if(lines[14][k] != ''):
             sum += float(lines[14][k])
             num += 1
    ave_me = sum/num
    for i in range(1,len(lines[3])):
        ...
        for j in range(1,6):#第j个用户的评分
            ...
            #print(co_user)
            if(lines[co_user][i] != ''):
                up += (float(lines[co_user][i]) - ave_s[j-1])*ans_out[j][0]
                down += ans_out[j][0]
        if(down != 0):
            ans_movie.insert(kk,(ave_me + (up/down),movies[i]))
        else:
